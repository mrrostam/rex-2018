function q_dot = quadrotor_model(q, t, u)
     ...
     zeta = [x;y;z]; %  position of the quadcopter
     zeta_dot = [x_dot;y_dot;z_dot];
     
     eta = [phi;theta;psi]; % angular position
     ...
     R = [cos(psi)*cos(theta) cos(psi)*sin(theta)*sin(phi)-sin(psi)*cos(phi) cos(psi)*sin(theta)*cos(phi)+sin(psi)*sin(phi);
      ...
      %  transformation matrix for angular velocities from the inertial frame to the body frame
     W_eta = [1 0 -sin(theta);
